(() => {
  ...
  function normalizeLayoutPath(value, fallbackName) {
    const path = String(value || "").replace(/\\/g, "/").replace(/^\/+/, "").trim();
    return /\.layout$/i.test(path) ? path : `UI/layouts/${fallbackName}.layout`;
  }

  function controllerSourceFor(classStem, layoutName, widgets, layoutGuid = "", layoutPath = "") {
    ...
    const normalizedLayoutGuid = normalizeLayoutGuid(layoutGuid);
    const layoutResourcePath = normalizeLayoutPath(layoutPath, layoutName);
    const layoutResourceName = normalizedLayoutGuid ? `{${normalizedLayoutGuid}}${layoutResourcePath}` : layoutResourcePath;
    const lines = [
      ...contextClasses,
      ...
      "{",
      `\tprotected static const ResourceName LAYOUT = \"${layoutResourceName}\";`,
    ...
    if (connected) {
      ...
      lines.push(
        ...
        "\t\tarray<int> playerIds = {};",
        "\t\tplayerManager.GetPlayers(playerIds);",
         "\t\tint enginePlayerCount = playerManager.GetPlayerCount();",
        "\t\tfor (int playerIndex = 0; playerIndex < playerIds.Count(); playerIndex++)",
        "\t\t{",
        "\t\t\tint playerId = playerIds[playerIndex];",
        "\t\t\tif (playerId <= 0)",
        ...
        "\t\tif (countText)",
         "\t\t\tcountText.SetText(enginePlayerCount.ToString() + \" CONNECTED\");",
  ...
  function makeWorkbenchPlan() {
    ...
    const layoutPath = normalizeLayoutPath(state.handoff.layoutPath, layoutName);
    ...
    return {
      ...
      target: {
        ...
        resourceGuid: layoutGuid || undefined,
        rootWidget: "m_wRoot",
        controllerClass,
        controllerPath
      },
  ...
  function validateHandoff() {
    ...
    if (!normalizeLayoutGuid(state.handoff.layoutGuid)) warnings.push("Target layout GUID is not set; register the target .layout in Workbench, paste its 16-character GUID into Handoff, and re-export before compiling the controller.");
    const assets = assetSummary();
  ...
  $("#workbenchMetaInput").addEventListener("change", event => {
    ...
    reader.onload = () => {
      try {
        ...
        state.handoff.layoutPath = meta.path.replace(/\\/g, "/");
