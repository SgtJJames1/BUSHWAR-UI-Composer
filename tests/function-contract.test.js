...
const updateMatches = source.match(/id: "ui\.widget\.update"/g) || [];
assert.strictEqual(updateMatches.length, 1, "widget update must have one canonical function entry");
...
assert(source.includes('method: "OnUpdate"'), "widget update must identify the native OnUpdate route");
assert(source.includes('id: "ui.widget.update"') && source.includes('requiresTarget: true'), "widget update must require an exact Workbench target widget");
