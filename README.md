...
- The `Export engine context snapshot` callback generates a local `$profile:BUSHWAR-UIComposer/runtime-context.json` file from the live controller. Import that file with **Import Workbench context** to preview the exact filtered PlayerManager rows, authoritative `PlayerManager.GetPlayerCount()` scalar, and GM editor state observed by Reforger; the file remains diagnostic evidence and never becomes runtime authority.
- The same context contract can carry the local GM editor state (`editorOpen`), `playerCount`, and `playerCountKnown`; scalar bindings display those captured values or explicit unknown-state labels instead of guessing. The player table remains filtered to valid positive IDs with non-empty names, so an engine count can never turn into an empty visual row.
